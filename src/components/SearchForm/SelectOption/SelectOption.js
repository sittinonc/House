      ...
      <div className={classes.king} onClick={dropdownHandler}>
        {props.type == "Location" ? (
          ...
          <span className={classes.topic}>
            {props.selectedPrice.length == 0
              ? "Price"
              : `> ${showMax(props.selectedPrice)}`}
          </span>
        )}
      </div>
              ...
              : props.data.map((e, i) => {
                  return (
                    <div
                      ...
                      className={classes.box}
                      onClick={() => {
                        if (includeCheck(props.selectedPrice, e) == 0) {
                          props.setSelectedPrice((prev) => [...prev, e]);
                        } else {
                          props.setSelectedPrice((prev) =>
                            excludeSelectedPrice(prev, e)
                          );
                        }
                      }}
                    >
                      ...
                      <div className={classes.icon}>
                        <FontAwesomeIcon
                          icon={
                            priceIconCheck(props.selectedPrice, e)
                              ? faCircleDot
                              : faCircle
                          }
                        />
